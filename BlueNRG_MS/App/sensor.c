  ...
  case EVT_VENDOR:
    {
      ...
      {
          /* * 最終精確索引：
           * data[0-1]: Connection Handle (如 0x0801)
           * data[2-3]: Attribute Handle  (如 0x0025)
           * data[4]:   Data Length       (2 bytes)
           * data[5-6]: Offset            (通常是 00 00)
           * data[7...]: Actual Data      <-- 這才是我們要的起點！
           */
          uint16_t attr_handle = (uint16_t)(blue_evt->data[2] | (blue_evt->data[3] << 8));
          uint8_t data_len = blue_evt->data[4];

          // 把這裡從 5 改成 7
          uint8_t *data_ptr = &blue_evt->data[7];

          extern void Attribute_Modified_CB(uint16_t handle, uint8_t data_length, uint8_t *att_data);
          Attribute_Modified_CB(attr_handle, data_len, data_ptr);
      }
      break;
      ...
      case EVT_BLUE_GATT_READ_PERMIT_REQ:
        ...
        break;
      } // 結束 switch(blue_evt->ecode)
    } // 結束 case EVT_VENDOR 的區塊
    break;

  } // 結束 switch(event_pckt->evt)
} // 結束 user_notify 函式
